...
public class HomePage extends JFrame{
	...
	public JPanel controller() {
		...
		showGridButton.setBackground(bga);
		showGridButton.setOpaque(true);
		showGridButton.setBorder(BorderFactory.createRaisedBevelBorder());
		showGridButton.setForeground(fg);
		showGridButton.getModel().addChangeListener(new ChangeListener() {
			@Override
			public void stateChanged(ChangeEvent e) {
				ButtonModel model = (ButtonModel) e.getSource();
                if (model.isPressed()) {
                	showGridButton.setBackground(bg);
                }else {
                	showGridButton.setBackground(bga);
                }
			}
		...
		c.gridx = 0;
		c.gridy = 3;
		buttonPanel.add(showGridButton, c);
